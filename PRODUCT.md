...
**Sticker administrators** (`role='admin'`) — staff who open and close the registration window, activate appointment dates, review each submitted application against its uploaded documents, approve or reject with a reason, and physically issue stickers by binding an RFID UID to an approved application at the sticker station. They also handle walk-in / emergency registrations that bypass the normal queue.
...
**Confirmed capabilities:** role-based registration and login with lockout and password reset; administrator-controlled registration window; 3-step application wizard with temp-file persistence between steps; document upload (OR/CR and driver's license always, COR for students, authorization letter for non-owners); automatic appointment assignment; administrator application list, detail review, approve/reject with reason; sticker station that binds a pending RFID UID to an approved application; quick/walk-in registration that creates the account and issues in one step; live gate view with today's entry/exit counts, active passes, hourly traffic, and latest scans; gate log browsing with CSV export; per-log incident report with PDF generation; time tracker for vehicles currently inside and overstays; immutable audit log across auth, application, gate, and admin events.

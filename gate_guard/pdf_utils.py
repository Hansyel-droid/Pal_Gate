...
def generate_incident_report_pdf(log):
    ...
    banner_style = ParagraphStyle('Banner', parent=styles['Normal'], fontSize=12, textColor=colors.white,
                                  backColor=colors.HexColor(status_colors.get(log.action, '#333')), alignment=1,
                                  spaceAfter=0.2 * inch)
    ...
    if log.rfid_tag and log.rfid_tag.sticker_application:
        ...
        vehicle_data.append(['Sticker ID', f"PSU-{app.id:04d}"])
